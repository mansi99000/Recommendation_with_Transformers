...
def evaluate(model: nn.Module, eval_data) -> float:
    ...
    with torch.no_grad():
        for i, (movie_data, user_data) in enumerate(eval_data):
            ...
            loss = criterion(output_flat, targets_flat)
            total_loss += loss.item()
    return total_loss / max(len(eval_data), 1)
...
def get_popular_movies(df_ratings):
    ...
    popular_movies = df_ratings.merge(rating_counts, on='movie_id')
...
if __name__ == "__main__":
    ...
    movies = data["movies"]
    movie_vocab = data["movie_vocab"]
    movie_vocab_stoi = data["movie_vocab_stoi"]
    user_vocab = data["user_vocab"]
    user_vocab_stoi = data["user_vocab_stoi"]
    movie_title_dict = data["movie_title_dict"]
    ...
    train_data, val_data, test_data = prepare_splits(ratings_data, sequence_length, step_size)
    ...
    # DataLoaders
    BATCH_SIZE = 256
    collate_batch = make_collate_fn(movie_vocab_stoi['<unk>'])
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Validation only — test set is held out until after all training
